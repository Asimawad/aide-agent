...
from .utils.self_reflection import perform_two_step_reflection # Removed perform_two_step_reflection_with_fewshot for now
...
console = Console() # For console output if needed, not for verbose file log
...
class Agent:
    ...
    def _prompt_environment(self):
        ...
        return {"Installed Packages": f"Your solution can use any relevant machine learning packages such as: {pkg_str}. Feel free to use any other packages too (all packages are already installed!). For neural networks we suggest using PyTorch rather than TensorFlow."}
    ...
    @property
    def _prompt_resp_fmt(self): # This seems unused now with specific plan/code/debug formats
        fmt = ("\n\n---\n"
               "1) PLAN (plain text, no fences):\n"
               "<your step‑by‑step reasoning here>\n\n"
               "2) CODE (one fenced Python block):\n"
               "```python\n<your python code here>\n```")
        return {"Response format": ("Your response should be a brief outline/sketch of your proposed solution in natural language (3-5 sentences), "
    ...
    def debug_prompt_resp_fmt(self):
        ...
        return {"Response format": ("Your response for the summary should be a detailed and high quality bullet points of the bugs in the previous solution, summarizing all the information and problems(5-7 sentences), "
    ...
    def code_prompt_resp_fmt(self):
        fmt = ("\n\n---\n"
               ...
               "```python\n<your python code here>\n```")
        return {"Response format": ("Your response should be a single markdown code block (wrapped in ```) which implements this solution and prints out the evaluation metric. "
    ...
    def plan_prompt_resp_fmt(self):
        ...
        return {"Response format": ("Your response for the summary should be a detailed and high quality bullet points of what the task is about, summarizing all the information in the task description (5-7 sentences), "
                                   "Your response for the plan should be a detailed and high quality bullet points of the steps of your proposed solution in natural language (7-10 sentences), "
    ...
    def _query_llm_with_retries(self, query_type: str, system_prompt: Any, user_prompt: Any, model: str, temperature: float, planner_flag: bool, current_step: int, convert_system_to_user: bool, retries: int = 3) -> Any:
        ...
        for attempt in range(retries):
            logger.info(f"{log_prefix}_ATTEMPT{attempt+1}: Sending request. Model: {model}, Temp: {temperature}, Planner: {planner_flag}", extra={"verbose": True})
            logger.debug(f"{log_prefix}_ATTEMPT{attempt+1}_SYSTEM_PROMPT_START\n{system_prompt}\n{log_prefix}_ATTEMPT{attempt+1}_SYSTEM_PROMPT_END", extra={"verbose": True})
            logger.debug(f"{log_prefix}_ATTEMPT{attempt+1}_USER_PROMPT_START\n{user_prompt}\n{log_prefix}_ATTEMPT{attempt+1}_USER_PROMPT_END", extra={"verbose": True})

            try:
                completion_text = backend_query(
                ...
                logger.debug(f"{log_prefix}_ATTEMPT{attempt+1}_RAW_RESPONSE_START\n{completion_text}\n{log_prefix}_ATTEMPT{attempt+1}_RAW_RESPONSE_END", extra={"verbose": True})
        ...
        return None # Should be unreachable if retries > 0 and an exception isn't raised
    ...
    def plan_query(self, prompt_user_message: Any, retries=3) -> tuple[str, str, str]:
        ...
        if completion_text is None:
            logger.error(f"{log_prefix}: Failed to get response after retries. Returning empty plan and summary.", extra={"verbose": True})
            return "", "", ""
        ...
        if plan and summary:
            ...
            logger.warning(f"{log_prefix}: Plan or summary extraction failed. Raw text: '{trim_long_string(completion_text if isinstance(completion_text, str) else str(completion_text))}'", extra={"verbose": True})
            logger.debug(f"{log_prefix}_EXTRACTION_FAILED_RAW_COMPLETION_START\n{completion_text}\n{log_prefix}_EXTRACTION_FAILED_RAW_COMPLETION_END", extra={"verbose": True})
            return "", completion_text if isinstance(completion_text, str) else str(completion_text), "" # Return raw text as plan if extraction fails
    ...
    def code_query(self, prompt_user_message: Any, retries=3) -> tuple[str, str, str]:
        ...
        if completion_text is None:
            logger.error(f"{log_prefix}: Failed to get response after retries. Returning empty code.", extra={"verbose": True})
            return "", "", ""
        ...
        if code:
            ...
            logger.warning(f"{log_prefix}: Code extraction failed. Raw text: '{trim_long_string(completion_text if isinstance(completion_text, str) else str(completion_text))}'", extra={"verbose": True})
            logger.debug(f"{log_prefix}_EXTRACTION_FAILED_RAW_COMPLETION_START\n{completion_text}\n{log_prefix}_EXTRACTION_FAILED_RAW_COMPLETION_END", extra={"verbose": True})
            # Return raw text as code if extraction fails, might be useful for debugging LLM output format
            return "", completion_text if isinstance(completion_text, str) else str(completion_text), ""


    def _draft(self, parent_node=None) -> Node:
        # console.rule(f"[cyan]Agent Step {self.current_step} - Stage : Drafting")
        logger.info(f"AGENT_DRAFT_STEP{self.current_step}: Starting drafting process. Parent: {parent_node.id if parent_node else 'None'}", extra={"verbose": True})
        ...
        logger.info(f"AGENT_DRAFT_STEP{self.current_step}: Calling plan_query for drafting.", extra={"verbose": True})
        ...
        if not agent_plan_for_step: # If plan generation failed
            logger.error(f"AGENT_DRAFT_STEP{self.current_step}: Plan generation failed. Cannot proceed with code generation.", extra={"verbose": True})
            # Create a dummy node indicating failure
            return Node(plan="PLAN GENERATION FAILED", code="# PLAN GENERATION FAILED", summary=agent_summary_for_step or "PLAN GENERATION FAILED", parent=parent_node)
        ...
        logger.info(f"AGENT_DRAFT_STEP{self.current_step}: Calling code_query for drafting.", extra={"verbose": True})
        ...
        if formatted_extracted_code:
            logger.debug(f"AGENT_DRAFT_STEP{self.current_step}_FORMATTED_GENERATED_CODE_START\n{formatted_extracted_code}\nAGENT_DRAFT_STEP{self.current_step}_FORMATTED_GENERATED_CODE_END", extra={"verbose": True})
        else:
            logger.warning(f"AGENT_DRAFT_STEP{self.current_step}: Code formatting failed for generated code.", extra={"verbose": True})
        ...
        logger.info(f"AGENT_DRAFT_STEP{self.current_step}: Drafted new node {new_node.id}.", extra={"verbose": True})
        return new_node

    def _improve(self, parent_node: Node) -> Node:
        # console.rule(f"[cyan]Stage : Improving")
        logger.info(f"AGENT_IMPROVE_STEP{self.current_step}: Starting improvement process for node {parent_node.id}.", extra={"verbose": True})
        ...
        # plan_prompt_user_message["Instructions"] |= self._prompt_environment
        if self.acfg.data_preview: # If planner needs data context for improvement ideas
            plan_prompt_user_message["Data Overview"] = self.data_preview


        logger.info(f"AGENT_IMPROVE_STEP{self.current_step}: Calling plan_query for improvement.", extra={"verbose": True})
        agent_summary_for_step, agent_plan_for_step, _ = self.plan_query(plan_prompt_user_message)

        if not agent_plan_for_step:
            logger.error(f"AGENT_IMPROVE_STEP{self.current_step}: Improvement plan generation failed for node {parent_node.id}.", extra={"verbose": True})
            return Node(plan="IMPROVEMENT PLAN GENERATION FAILED", code=parent_node.code, summary=agent_summary_for_step or "IMPROVEMENT PLAN FAILED", parent=parent_node)


        # --- Code Generation for Improvement ---
        code_prompt_user_message: Any = { # Renamed from 'prompt' to 'code_prompt_user_message'
            "Introduction": code_introduction,
            "Task description summary and previous solution": agent_summary_for_step, # Use summary from planning
            "Improvement plan": {"Plan": agent_plan_for_step}, # Crucial: Add the generated plan
            "Previous solution code": {"Code": wrap_code(parent_node.code)}, # Keep original code for context
        ...
        code_prompt_user_message["Instructions"] |= {
            ...
            "additional guidelines": self._prompt_impl_guideline["Implementation Guideline"] # Reuse
        }
        if self.acfg.data_preview:
            code_prompt_user_message["Data Overview"] = self.data_preview

        logger.info(f"AGENT_IMPROVE_STEP{self.current_step}: Calling code_query for improvement.", extra={"verbose": True})
        _, generated_code, _ = self.code_query(code_prompt_user_message)

        formatted_extracted_code = format_code(generated_code)
        if formatted_extracted_code:
            logger.debug(f"AGENT_IMPROVE_STEP{self.current_step}_FORMATTED_GENERATED_CODE_START\n{formatted_extracted_code}\nAGENT_IMPROVE_STEP{self.current_step}_FORMATTED_GENERATED_CODE_END", extra={"verbose": True})

        new_node = Node(plan=agent_plan_for_step, code=generated_code, summary=agent_summary_for_step, parent=parent_node)
        logger.info(f"AGENT_IMPROVE_STEP{self.current_step}: Improved node {parent_node.id} to create new node {new_node.id}.", extra={"verbose": True})
        return new_node

    def _debug(self, parent_node: Node) -> Node:
        # console.rule(f"[cyan]Stage : Debugging")
        logger.info(f"AGENT_DEBUG_STEP{self.current_step}: Starting debugging process for node {parent_node.id}.", extra={"verbose": True})
        logger.debug(f"AGENT_DEBUG_STEP{self.current_step}_PARENT_CODE_START\n{parent_node.code}\nAGENT_DEBUG_STEP{self.current_step}_PARENT_CODE_END", extra={"verbose": True})
        logger.debug(f"AGENT_DEBUG_STEP{self.current_step}_PARENT_TERM_OUT_START\n{parent_node.term_out}\nAGENT_DEBUG_STEP{self.current_step}_PARENT_TERM_OUT_END", extra={"verbose": True})
        ...
        logger.info(f"AGENT_DEBUG_STEP{self.current_step}: Calling plan_query for debugging.", extra={"verbose": True})
        agent_summary_for_step, agent_plan_for_step, _ = self.plan_query(plan_prompt_user_message)

        if not agent_plan_for_step:
            logger.error(f"AGENT_DEBUG_STEP{self.current_step}: Debug plan generation failed for node {parent_node.id}.", extra={"verbose": True})
            return Node(plan="DEBUG PLAN GENERATION FAILED", code=parent_node.code, summary=agent_summary_for_step or "DEBUG PLAN FAILED", parent=parent_node)
        ...
        code_prompt_user_message["Instructions"] |= {
            ...
            "additional guidelines": self._prompt_impl_guideline["Implementation Guideline"]
        }
        if self.acfg.data_preview:
            code_prompt_user_message["Data Overview"] = self.data_preview


        logger.info(f"AGENT_DEBUG_STEP{self.current_step}: Calling code_query for debugging.", extra={"verbose": True})
        _, generated_code, _ = self.code_query(code_prompt_user_message)

        formatted_extracted_code = format_code(generated_code)
        if formatted_extracted_code:
            logger.debug(f"AGENT_DEBUG_STEP{self.current_step}_FORMATTED_GENERATED_CODE_START\n{formatted_extracted_code}\nAGENT_DEBUG_STEP{self.current_step}_FORMATTED_GENERATED_CODE_END", extra={"verbose": True})


        new_node = Node(plan=agent_plan_for_step, code=generated_code, summary=agent_summary_for_step, parent=parent_node)
        logger.info(f"AGENT_DEBUG_STEP{self.current_step}: Debugged node {parent_node.id} to create new node {new_node.id}.", extra={"verbose": True})
        return new_node

    def reflect(self, node: Node) -> tuple[str, str]:
        logger.info(f"AGENT_REFLECT_STEP{self.current_step}: Initiating self-reflection for node {node.id}.", extra={"verbose": True})
        logger.debug(f"AGENT_REFLECT_STEP{self.current_step}_NODE_CODE_START\n{node.code}\nAGENT_REFLECT_STEP{self.current_step}_NODE_CODE_END", extra={"verbose": True})
        logger.debug(f"AGENT_REFLECT_STEP{self.current_step}_NODE_ANALYSIS_START\n{node.analysis}\nAGENT_REFLECT_STEP{self.current_step}_NODE_ANALYSIS_END", extra={"verbose": True})
        logger.debug(f"AGENT_REFLECT_STEP{self.current_step}_NODE_TERM_OUT_START\n{node.term_out}\nAGENT_REFLECT_STEP{self.current_step}_NODE_TERM_OUT_END", extra={"verbose": True})
        ...
        try:
            ...
            logger.info(f"AGENT_REFLECT_STEP{self.current_step}: Self-reflection completed.", extra={"verbose": True})
            logger.debug(f"AGENT_REFLECT_STEP{self.current_step}_REFLECTION_PLAN_START\n{reflection_plan}\nAGENT_REFLECT_STEP{self.current_step}_REFLECTION_PLAN_END", extra={"verbose": True})
            logger.debug(f"AGENT_REFLECT_STEP{self.current_step}_REVISED_CODE_START\n{revised_code}\nAGENT_REFLECT_STEP{self.current_step}_REVISED_CODE_END", extra={"verbose": True})

        except Exception as e:
            logger.error(f"AGENT_REFLECT_STEP{self.current_step}: Error during self-reflection: {e}", exc_info=True, extra={"verbose": True})
            return "REFLECTION_ERROR: " + str(e), node.code # Return original code on error

        if revised_code and revised_code.strip() and revised_code != node.code:
            logger.info(f"AGENT_REFLECT_STEP{self.current_step}: Self-reflection resulted in code changes.", extra={"verbose": True})
        elif reflection_plan == "No specific errors found requiring changes.":
            logger.info(f"AGENT_REFLECT_STEP{self.current_step}: Self-reflection found no errors requiring changes.", extra={"verbose": True})
        else:
            logger.warning(f"AGENT_REFLECT_STEP{self.current_step}: Self-reflection finished, but revised code is same as original or empty. Plan: {reflection_plan}", extra={"verbose": True})
    ...
    def summarize_task(self, task_desc: str) -> str:
        ...
        logger.info(f"{log_prefix}: Summarizing task description.", extra={"verbose": True})
        ...
        user_prompt = {
            ...
            "Instructions": ("Please provide a concise summary ... Limit the summary to around 7 sentences.") # Truncated
        }
        ...
        if summary_completion is None:
            logger.error(f"{log_prefix}: Failed to get summary after retries. Returning original task description.", extra={"verbose": True})
        ...
        logger.debug(f"{log_prefix}_CONCISE_SUMMARY_START\n{concise_summary.strip()}\n{log_prefix}_CONCISE_SUMMARY_END", extra={"verbose": True})
        return concise_summary.strip()

    def update_data_preview(self):
        logger.info(f"AGENT_STEP{self.current_step}: Updating data preview.", extra={"verbose": True})
        try:
            self.data_preview = data_preview.generate(self.cfg.workspace_dir)
            logger.info(f"AGENT_STEP{self.current_step}: Data preview updated.", extra={"verbose": True})
            logger.debug(f"AGENT_STEP{self.current_step}_DATA_PREVIEW_START\n{self.data_preview}\nAGENT_STEP{self.current_step}_DATA_PREVIEW_END", extra={"verbose": True})
        except Exception as e:
            logger.error(f"AGENT_STEP{self.current_step}: Failed to update data preview: {e}", exc_info=True, extra={"verbose": True})
            self.data_preview = "Error generating data preview."


    def step(self, exec_callback: ExecCallbackType, current_step_number: int):
        logger.info(f"AGENT_STEP_START: {current_step_number}, Total Steps: {self.acfg.steps}", extra={"verbose": True})
        t_step_start = time.time()

        submission_dir = self.cfg.workspace_dir / "submission"
        logger.info(f"AGENT_STEP{current_step_number}: Clearing submission directory: {submission_dir}", extra={"verbose": True})
        ...
        logger.info(f"AGENT_STEP{current_step_number}: Calling search_policy.", extra={"verbose": True})
        ...
        if parent_node is None:
            draft_flag = True
            node_stage = "draft"
            logger.info(f"AGENT_STEP{current_step_number}: Stage selected: DRAFTING.", extra={"verbose": True})
            result_node = self._draft(parent_node)
        elif parent_node.is_buggy:
            node_stage = "debug"
            logger.info(f"AGENT_STEP{current_step_number}: Stage selected: DEBUGGING node {parent_node.id}.", extra={"verbose": True})
            result_node = self._debug(parent_node)
        else:
            node_stage = "improve"
            logger.info(f"AGENT_STEP{current_step_number}: Stage selected: IMPROVING node {parent_node.id}.", extra={"verbose": True})
            result_node = self._improve(parent_node)

        logger.info(f"AGENT_STEP{current_step_number}: Executing code for node {result_node.id} (stage: {node_stage}). Code length: {len(result_node.code)}", extra={"verbose": True})
        logger.debug(f"AGENT_STEP{current_step_number}_CODE_TO_EXECUTE_START\n{result_node.code}\nAGENT_STEP{current_step_number}_CODE_TO_EXECUTE_END", extra={"verbose": True})
        ...
        if draft_flag and self.acfg.ITS_Strategy == "self-reflection" and result_node.is_buggy:
            logger.info(f"AGENT_STEP{current_step_number}: Condition met for self-reflection on drafted buggy node {result_node.id}.", extra={"verbose": True})
            ...
            try:
                ...
                if reflection_code and reflection_code.strip() and reflection_code != result_node.code:
                    logger.info(f"AGENT_STEP{current_step_number}: Self-reflection yielded new code for node {result_node.id}. Re-executing.", extra={"verbose": True})
                    result_node.code = reflection_code # Update node's code
                    reflection_applied = True

                    logger.info(f"AGENT_STEP{current_step_number}: Re-executing reflected code for node {result_node.id}. Code length: {len(result_node.code)}", extra={"verbose": True})
                    logger.debug(f"AGENT_STEP{current_step_number}_REFLECTED_CODE_TO_EXECUTE_START\n{result_node.code}\nAGENT_STEP{current_step_number}_REFLECTED_CODE_TO_EXECUTE_END", extra={"verbose": True})
                    exec_start_time = time.time()
                    exec_result = exec_callback(result_node.code, reset_session=True)
                    ...
                    logger.info(f"AGENT_STEP{current_step_number}: Self-reflection did not result in applicable code changes for node {result_node.id}.", extra={"verbose": True})
            except Exception as e: # Catch errors specifically from reflection process
                logger.error(f"AGENT_STEP{current_step_number}: Error during self-reflection stage for node {result_node.id}: {e}", exc_info=True, extra={"verbose": True})

        # Update effectiveness flags
        if self._prev_buggy and not result_node.is_buggy: # If it *was* buggy (before reflection if any) and now is *not*
            result_node.effective_debug_step = True # This might mean the main debug/improve worked, or reflection worked
            result_node.effective_reflections = reflection_applied # True only if reflection was applied and fixed it
        else:
        ...
        if best_node and best_node.id == result_node.id:
             logger.info(f"AGENT_STEP{current_step_number}: Node {result_node.id} is new best node (Metric: {best_node.metric.value if best_node.metric else 'N/A':.4f}). Caching solution.", extra={"verbose": True})
             best_solution_dir = self.cfg.workspace_dir / "best_solution"
             best_submission_dir = self.cfg.workspace_dir / "best_submission"
             best_solution_dir.mkdir(exist_ok=True, parents=True)
             best_submission_dir.mkdir(exist_ok=True, parents=True)
             if submission_exists:
                 shutil.copy(submission_path, best_submission_dir / "submission.csv") # Add filename
             best_code_path = best_solution_dir / "solution.py"
             with open(best_code_path, "w") as f: f.write(result_node.code)
             with open(best_solution_dir / "node_id.txt", "w") as f: f.write(str(result_node.id))
        elif best_node:
             logger.info(f"AGENT_STEP{current_step_number}: Current best node is {best_node.id} (Metric: {best_node.metric.value if best_node.metric else 'N/A':.4f})", extra={"verbose": True})
        else:
            logger.info(f"AGENT_STEP{current_step_number}: No best node identified yet.", extra={"verbose": True})
        ...
        logger.info(f"AGENT_STEP_END: {current_step_number}, Duration: {t_step_end - t_step_start:.2f}s", extra={"verbose": True})
    ...
    def parse_exec_result(self, node: Node, exec_result: ExecutionResult) -> Node:
        ...
        logger.debug(f"{log_prefix}_ABSORBED_EXEC_RESULT_NODE_EXC_TYPE: {node.exc_type}", extra={"verbose": True})
        logger.debug(f"{log_prefix}_ABSORBED_EXEC_RESULT_NODE_TERM_OUT_START\n{node.term_out}\n{log_prefix}_ABSORBED_EXEC_RESULT_NODE_TERM_OUT_END", extra={"verbose": True})
        ...
        for attempt in range(max_retries):
            ...
            try:
                ...
                raw_response = backend_query(
                    ...
                    # excute = False, # 'excute' param not in backend_query, assume func_spec implies it
                    convert_system_to_user=self.acfg.convert_system_to_user,
                    current_step=self.current_step, # Pass current_step
                )
                logger.info(f"{log_prefix}_FEEDBACK_LLM_ATTEMPT{attempt+1}: Received response.", extra={"verbose": True})
                ...
                if not isinstance(raw_response, dict):
                    logger.error(f"{log_prefix}_FEEDBACK_LLM_ATTEMPT{attempt+1}: Response is not a dict as expected for function call. Type: {type(raw_response)}", extra={"verbose": True})
                    # Try to parse if it's a string that looks like JSON (common failure mode)
                    if isinstance(raw_response, str):
                        try:
                            import json
                            raw_response = json.loads(raw_response)
                            if not isinstance(raw_response, dict): # Still not a dict after parsing
                ...
                review_response_dict = cast(dict, raw_response) if isinstance(raw_response, dict) else None


                if review_response_dict and all(k in review_response_dict for k in ["is_bug", "has_csv_submission", "summary", "metric", "lower_is_better", "code_quality"]):
                    ...
                    logger.warning(f"{log_prefix}_FEEDBACK_LLM_ATTEMPT{attempt+1}: Feedback LLM response missing required keys or is None. Response: {review_response_dict}", extra={"verbose": True})
        ...
        if not isinstance(metric_value, (float, int)):
            if metric_value is not None: # Log if it was something else
                 logger.warning(f"{log_prefix}: Metric value from LLM is not a float/int: '{metric_value}' (type: {type(metric_value)}). Setting to None.", extra={"verbose": True})
            metric_value = None
        ...
        if not isinstance(self._code_quality, (int, float)): # Ensure it's a number
            logger.warning(f"{log_prefix}: Code quality from LLM is not an int/float: '{self._code_quality}'. Setting to 0.", extra={"verbose": True})
        ...
        node.is_buggy = (
            is_bug_llm or exc_type_exists or metric_missing or
            csv_missing_reported or csv_missing_actual
        )
        logger.info(f"{log_prefix}: Final buggy status: {node.is_buggy}", extra={"verbose": True})

        if node.is_buggy:
            bug_reasons = []
            if is_bug_llm: bug_reasons.append(f"LLM_judged_buggy (Summary: {node.analysis})")
            ...
            if csv_missing_reported: bug_reasons.append("LLM_reported_CSV_missing")
            if csv_missing_actual: bug_reasons.append("Actual_CSV_file_missing")
            logger.info(f"{log_prefix}: Reasons for buggy status: {'; '.join(bug_reasons)}", extra={"verbose": True})
            node.metric = WorstMetricValue()
        else:
            logger.info(f"{log_prefix}: Node determined not buggy. Metric value: {metric_value}, Lower_is_better: {not review_response_dict.get('lower_is_better', True)}", extra={"verbose": True})
