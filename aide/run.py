...
def run():
    ...
    logger.info(
        f"Logging initialized. verbose.log level: DEBUG, aide.log level: INFO, Console level: {cfg.log_level.upper()}."
    )
    ...
    try:
        ...
        logger.warning(
            f"No benchmarks found for competition: {cfg.competition_name}. Medal/threshold logging might be affected."
        )
    ...
    if cfg.agent.ITS_Strategy == "planner":
        ...
        cfg.agent.ITS_Strategy == "codechain"
        or cfg.agent.ITS_Strategy == "codechain_v2"
        or cfg.agent.ITS_Strategy == "codechain_v3"
    ):
    ...
    try:
        with Progress(
            ...
            for i in range(cfg.agent.steps):
                ...
                logger.info(
                    f"--- Agent Step {current_step_num_display}/{cfg.agent.steps} START ---"
                )
                t0_step = time.time()

                # The agent.step method will internally call wandb_logger_instance.log_step_data
                agent.step(
                    exec_callback=exec_callback, current_step_number=current_step_num_display
                )

                t1_step = time.time()
                logger.info(
                    f"Step {current_step_num_display} processing took {t1_step - t0_step:.2f} seconds."
                )
                ...
                if (current_step_num_display % cfg.get("save_every_n_steps", 1)) == 0:
                    ...
                    save_run(
                        cfg, journal
                    )  # This saves journal.json, config.yaml, tree_plot.html locally

                global_step += 1
                progress_bar.update(main_task, advance=1)
                logger.info(
                    f"--- Agent Step {current_step_num_display}/{cfg.agent.steps} END (Total time: {t1_step - t0_step:.2f}s) --- \n"
                )
        ...
        if wandb_logger_instance and wandb_logger_instance.wandb_run:
            try:
                ...
                if active_wandb_run_id:
                    logger.info(
                        f"Successfully obtained active W&B run ID for metrics calculation: {active_wandb_run_id}"
                    )
                else:  # Should not happen if .id exists and is not None
                    logger.warning("W&B run object exists but ID is None or empty.")
            except AttributeError:
                logger.warning(
                    "W&B run object (wandb_logger_instance.wandb_run) is None or lacks 'id' attribute. Cannot pass ID to metrics calculator."
                )
            ...
            logger.info(
                "WandbLogger or its run object is not available. Proceeding without W&B run ID for metrics calculation."
            )
        ...
        try:
            ...
            generate_all_metrics(
                cfg.exp_name, run_id_for_wandb=active_wandb_run_id  # Pass the ID if available
            )
        ...
        if global_step == 0 or global_step == cfg.agent.steps:
            try:
                if global_step == cfg.agent.steps and (cfg.workspace_dir / "input").exists():
                    logger.info(
                        f"Cleaning up input directory in workspace: {cfg.workspace_dir / 'input'}"
                    )
                    shutil.rmtree(cfg.workspace_dir / "input")
                elif global_step == 0 and cfg.workspace_dir.exists():
                    logger.info(
                        f"Cleaning up entire workspace as no steps completed: {cfg.workspace_dir}"
                    )
                    shutil.rmtree(cfg.workspace_dir)
                ...
                if submission_history_dir_to_clean.exists():
                    if global_step == cfg.agent.steps:
                        logger.info(
                            f"Cleaning up submission_history_per_step directory: {submission_history_dir_to_clean}"
                        )
