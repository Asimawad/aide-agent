...
ExtractCodeFuncType = Callable[[Any], Optional[str]] # Response might not be str, result can be None
...
def perform_two_step_reflection(
    ...
    logger.info(f"{log_prefix_critique}: Sending request for critique and edit proposal. Model: {cfg.agent.code.planner_model}", extra={"verbose": True})
    logger.debug(f"{log_prefix_critique}_SYSTEM_PROMPT_START\n{system_prompt1}\n{log_prefix_critique}_SYSTEM_PROMPT_END", extra={"verbose": True})
    logger.debug(f"{log_prefix_critique}_USER_PROMPT_START\n{critique_user_prompt}\n{log_prefix_critique}_USER_PROMPT_END", extra={"verbose": True})
    ...
    try:
        ...
        logger.info(f"{log_prefix_critique}: Received critique response.", extra={"verbose": True})
        logger.debug(f"{log_prefix_critique}_RAW_RESPONSE_START\n{plan_raw}\n{log_prefix_critique}_RAW_RESPONSE_END", extra={"verbose": True})
    ...
    if "No specific errors found requiring changes." in reflection_plan:
        logger.info(f"{log_prefix_main}: Critique found no specific errors. Returning original code.", extra={"verbose": True})
    ...
    logger.info(f"{log_prefix_coder}: Sending request for code revision. Model: {model_name}", extra={"verbose": True}) # Using model_name passed in, which is acfg.code.model
    logger.debug(f"{log_prefix_coder}_SYSTEM_PROMPT_START\n{system_prompt2}\n{log_prefix_coder}_SYSTEM_PROMPT_END", extra={"verbose": True})
    logger.debug(f"{log_prefix_coder}_USER_PROMPT_START\n{coder_user_prompt}\n{log_prefix_coder}_USER_PROMPT_END", extra={"verbose": True})
    ...
    try:
        ...
        logger.info(f"{log_prefix_coder}: Received code revision response.", extra={"verbose": True})
        logger.debug(f"{log_prefix_coder}_RAW_RESPONSE_START\n{revised_code_response}\n{log_prefix_coder}_RAW_RESPONSE_END", extra={"verbose": True})
    except Exception as e:
        logger.error(f"{log_prefix_coder}: Error during coder LLM query: {e}", exc_info=True, extra={"verbose": True})
        return reflection_plan, code # Return original code on error

    revised_code = extract_code_func(revised_code_response)
    if revised_code:
        logger.debug(f"{log_prefix_coder}_EXTRACTED_CODE_START\n{revised_code}\n{log_prefix_coder}_EXTRACTED_CODE_END", extra={"verbose": True})
    else:
        logger.warning(f"{log_prefix_coder}: Code extraction failed from revision response. Raw response was: {revised_code_response}", extra={"verbose": True})


    final_revised_code = revised_code if revised_code and revised_code.strip() else code
    if final_revised_code == code and revised_code and revised_code.strip(): # It extracted something but it was same as original or whitespace
         logger.info(f"{log_prefix_main}: Extracted revised code is same as original or empty after stripping. Using original code.", extra={"verbose": True})
    elif not revised_code or not revised_code.strip():
         logger.info(f"{log_prefix_main}: No valid revised code extracted. Using original code.", extra={"verbose": True})
    else:
