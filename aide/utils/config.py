...
from typing import Hashable, cast, Optional, Dict, Any, List # Ensure Optional, Dict, Any, List are imported
import json
...
class SearchConfig:
    ...
    num_drafts: int
@dataclass
class ToTPhaseConfig:
    """Configuration for a single phase of Tree of Thoughts (e.g., planning or segment coding)."""
    enabled: bool = False
    method_generate: str = "propose"  # Choices: "sample", "propose"
    method_evaluate: str = "value"   # Choices: "value", "vote"
    method_select: str = "greedy"     # Choices: "greedy" (initially)
    n_generate_sample: int = 3       # k: Number of thoughts/plans/snippets to generate
    n_evaluate_sample: int = 1       # Number of LLM calls for evaluation (for robustness)
    n_select_sample: int = 1         # b: Beam width / number of candidates to keep
...
class ToTConfig:
    ...
    planning: ToTPhaseConfig = field(default_factory=ToTPhaseConfig)
    segment_coding: ToTPhaseConfig = field(default_factory=ToTPhaseConfig)
...
class AgentConfig:
    steps: int
    time_limit: int
    k_fold_validation: int # Assuming this was intended to be part of AgentConfig
    expose_prediction: bool
    data_preview: bool
    convert_system_to_user: bool
    obfuscate: bool
    ITS_Strategy: str
    code: StageConfig       # For primary coder/planner model settings
    feedback: StageConfig   # For feedback/review LLM settings
    search: SearchConfig
    tot: ToTConfig = field(default_factory=ToTConfig) # <<< ToT config integrated here
