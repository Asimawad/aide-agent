...
class WandbLogger:
    def __init__(
        ...
        self._sub_produced_flags: list[int] = (
            []
        )  # 1 if submission.csv produced by non-buggy code this step, else 0
        ...
        self._effective_fix_flags: list[int] = (
            []
        )  # 1 if a fix (debug/reflection) was effective this step
        self._effective_reflection_flags: list[int] = (
            []
        )  # 1 if reflection specifically led to a fix
    ...
    def init_wandb(self):
        if wandb and OmegaConf and self.cfg.wandb.enabled:
            try:
                resolved_cfg_container = OmegaConf.to_container(
                ...
                self.wandb_run = wandb.init(
                    ...
                    tags=[
                        "aide-agent",
                        self.cfg.agent.ITS_Strategy,
                        self.cfg.agent.code.model,
                        self.cfg.competition_name,
                    ],
                )
                self.app_logger.info(
                    f"W&B run initialized: {self.wandb_run.url if self.wandb_run else 'Failed'}"
                )
    ...
    def log_step_data(
        ...
        current_submission_dir: Path,
    ):  # Path to ./submission for current step
        ...
        try:
            ...
            submission_produced_this_step = (
                1 if submission_csv_current_step.exists() and not is_buggy_final else 0
            )
            ...
            if submission_produced_this_step == 1:
                try:
                    archive_sub_path = (
                        self.submission_history_dir / f"step_{current_step_number}_submission.csv"
                    )
                    shutil.copy2(submission_csv_current_step, archive_sub_path)
                    logger.debug(
                        f"{log_prefix_step}: Archived submission to {archive_sub_path}",
                        extra={"verbose": True},
                    )
                except Exception as e_copy:
                    logger.error(
                        f"{log_prefix_step}: Error archiving submission for step {current_step_number}: {e_copy}"
                    )

            # Update scalars in current_log_data to reflect final state
            current_log_data["eval/is_buggy"] = 1 if is_buggy_final else 0
            current_log_data["eval/submission_produced_this_step"] = (
                submission_produced_this_step  # Log for this specific step
            )
            ...
            self._sub_produced_flags.append(
                submission_produced_this_step
            )  # Tracks if sub was made by non-buggy code
            ...
            if not is_buggy_final and not pd.isna(metric_val_final):
                ...
                if self.competition_benchmarks:
                    logger.debug(
                        f"{log_prefix_step}: Calculating medals. Metric: {metric_val_final:.4f}",
                        extra={"verbose": True},
                    )
                    ...
                    logger.debug(
                        f"{log_prefix_step}: Medals for this step - Median: {above_median_achieved}, Gold: {gold_achieved}, Silver: {silver_achieved}, Bronze: {bronze_achieved}",
                        extra={"verbose": True},
                    )
                else:
                    logger.warning(
                        f"{log_prefix_step}: No competition_benchmarks available, cannot calculate medal flags for this step.",
                        extra={"verbose": True},
                    )
                    self._above_median_flags.append(0)
                    self._gold_medal_flags.append(0)
                    self._silver_medal_flags.append(0)
                    self._bronze_medal_flags.append(0)
            else:
                self._above_median_flags.append(0)
                self._gold_medal_flags.append(0)
                self._silver_medal_flags.append(0)
            ...
            if self._bug_flags:  # Check if list is not empty
                ...
                bug_table = wandb.Table(
                    data=[["Buggy", bug_count], ["Clean", clean_count]], columns=["label", "count"]
                )
                current_log_data["plots/bug_vs_clean_cumulative"] = wandb.plot.bar(
                    bug_table, "label", "count", title="Buggy vs Clean Steps (Cumulative)"
                )
            ...
            if self._sub_produced_flags:
                ...
                not_produced_count = (
                    len(self._sub_produced_flags) - produced_count
                )  # Count of non-buggy steps that didn't produce
                sub_table = wandb.Table(
                    data=[
                        ["Submission Produced (Non-Buggy)", produced_count],
                        ["No Submission (Non-Buggy)", not_produced_count],
                    ],
                    columns=["label", "count"],
                )
                current_log_data["plots/submission_presence_cumulative"] = wandb.plot.bar(
                    sub_table,
                    "label",
                    "count",
                    title="Submission Presence by Non-Buggy Steps (Cumulative)",
                )

            # Medals (Cumulative)
            if self.competition_benchmarks:
                if self._above_median_flags:
                    true_count = sum(self._above_median_flags)
                    false_count = len(self._above_median_flags) - true_count
                    table = wandb.Table(
                        data=[["Above Median", true_count], ["Not Above Median", false_count]],
                        columns=["label", "count"],
                    )
                    current_log_data["plots/above_median_bar_cumulative"] = wandb.plot.bar(
                        table, "label", "count", title="Above Median Steps (Cumulative)"
                    )
                if self._gold_medal_flags:
                    true_count = sum(self._gold_medal_flags)
                    false_count = len(self._gold_medal_flags) - true_count
                    table = wandb.Table(
                        data=[["Gold Medal", true_count], ["No Gold Medal", false_count]],
                        columns=["label", "count"],
                    )
                    current_log_data["plots/gold_medal_bar_cumulative"] = wandb.plot.bar(
                        table, "label", "count", title="Gold Medal Steps (Cumulative)"
                    )
                if self._silver_medal_flags:
                    true_count = sum(self._silver_medal_flags)
                    false_count = len(self._silver_medal_flags) - true_count
                    table = wandb.Table(
                        data=[["Silver Medal", true_count], ["No Silver Medal", false_count]],
                        columns=["label", "count"],
                    )
                    current_log_data["plots/silver_medal_bar_cumulative"] = wandb.plot.bar(
                        table, "label", "count", title="Silver Medal Steps (Cumulative)"
                    )
                if self._bronze_medal_flags:
                    true_count = sum(self._bronze_medal_flags)
                    false_count = len(self._bronze_medal_flags) - true_count
                    table = wandb.Table(
                        data=[["Bronze Medal", true_count], ["No Bronze Medal", false_count]],
                        columns=["label", "count"],
                    )
                    current_log_data["plots/bronze_medal_bar_cumulative"] = wandb.plot.bar(
                        table, "label", "count", title="Bronze Medal Steps (Cumulative)"
                    )

            # Metric History Scatter
            if self._metric_hist:
                valid_metrics_for_plot = [
                    m for m in self._metric_hist if isinstance(m, (int, float)) and not pd.isna(m)
                ]
                if valid_metrics_for_plot:
                    metric_scatter_data = [[i, m] for i, m in enumerate(valid_metrics_for_plot)]
                    if metric_scatter_data:
                        tbl = wandb.Table(
                            data=metric_scatter_data, columns=["step_idx_non_buggy", "metric_value"]
                        )
                        current_log_data["plots/val_metric_history_scatter"] = wandb.plot.scatter(
                            tbl,
                            "step_idx_non_buggy",
                            "metric_value",
                            title="Validation Metric History (Non-Buggy Steps)",
                        )
            ...
            logger.debug(
                f"W&B: Logged step {current_step_number} data. Keys: {list(current_log_data.keys())}",
                extra={"verbose": True},
            )
        except Exception as e:
            logger.error(
                f"W&B: Error logging step data for step {current_step_number}: {e}", exc_info=True
            )
    ...
    def _stage_files_for_wandb_artifacts(self):
        ...
        best_solution_dst_stage = (
            wandb_artifacts_staging_dir / "best_solution_code"
        )  # Subdir in staging
        ...
        best_submission_dst_stage = (
            wandb_artifacts_staging_dir / "best_submission_file"
        )  # Subdir in staging
        if best_submission_src_ws.exists() and any(best_submission_src_ws.iterdir()):
            ...
            logger.info(
                f"W&B Staging: Copied best_submission folder to {best_submission_dst_stage}"
            )
        ...
        if submission_history_src.exists() and any(submission_history_src.iterdir()):
            shutil.copytree(
                submission_history_src, submission_history_dst_stage, dirs_exist_ok=True
            )
            logger.info(
                f"W&B Staging: Copied submission_history_per_step to {submission_history_dst_stage}"
            )
    ...
    def finalize_run(self, journal: Journal):  # Removed competition_benchmarks from args, use self.
        ...
        try:
            ...
            summary_data = {}
            wo_step = None
            num_working_code_steps = 0
            buggy_nodes_count = 0
            total_code_quality_non_buggy = 0
            count_code_quality_non_buggy = 0
            ...
            summary_data["summary/steps_to_first_working_code"] = (
                (wo_step + 1) if wo_step is not None else (self.cfg.agent.steps + 10)
            )
            summary_data["summary/num_working_code_steps_total"] = (
                num_working_code_steps  # Total non-buggy steps
            )
            summary_data["summary/num_buggy_nodes_total"] = buggy_nodes_count
            summary_data["summary/avg_code_quality_non_buggy_nodes"] = (
                (total_code_quality_non_buggy / count_code_quality_non_buggy)
                if count_code_quality_non_buggy > 0
                else 0
            )
            ...
            summary_data["summary/total_effective_reflection_fixes_in_run"] = sum(
                self._effective_reflection_flags
            )
            summary_data["summary/total_submissions_by_non_buggy_nodes"] = sum(
                self._sub_produced_flags
            )
            ...
            if best_node and best_node.metric and best_node.metric.value is not None:
                ...
                best_node_submission_path_ws = (
                    self.cfg.workspace_dir / "best_submission" / "submission.csv"
                )
                summary_data["summary/best_overall_solution_produced_submission_csv"] = (
                    1 if best_node_submission_path_ws.exists() else 0
                )

            self.wandb_run.summary.update(summary_data)
            self.app_logger.info(
                f"W&B: Updated run summary with final aggregations: {summary_data}"
            )
            ...
            if (wandb_artifacts_staging_dir / "journal.json").exists():
                artifact_journal = wandb.Artifact(
                    f"{sanitized_exp_name[:100]}_run_journal", type="run-journal"
                )
                artifact_journal.add_file(str(wandb_artifacts_staging_dir / "journal.json"))
                self.wandb_run.log_artifact(artifact_journal)
                self.app_logger.info("W&B: Logged run_journal artifact.")
            ...
            if best_solution_pkg_path.exists() and any(best_solution_pkg_path.iterdir()):
                artifact_code = wandb.Artifact(
                    f"{sanitized_exp_name[:100]}_best_solution_package", type="solution-package"
                )
                artifact_code.add_dir(str(best_solution_pkg_path))
                self.wandb_run.log_artifact(artifact_code)
                self.app_logger.info("W&B: Logged best_solution_package artifact.")

            # Log Best Submission File(s) package as an artifact
            best_submission_pkg_path = (
                wandb_artifacts_staging_dir / "best_submission_file"
            )  # Name matches staging
            if best_submission_pkg_path.exists() and any(best_submission_pkg_path.iterdir()):
                artifact_submission = wandb.Artifact(
                    f"{sanitized_exp_name[:100]}_best_submission_package", type="submission-package"
                )
                artifact_submission.add_dir(str(best_submission_pkg_path))
                self.wandb_run.log_artifact(artifact_submission)
                self.app_logger.info("W&B: Logged best_submission_package artifact.")

            # Log Submission History (all CSVs from non-buggy steps) as an artifact
            all_submissions_history_path = (
                wandb_artifacts_staging_dir / "all_step_submissions_history"
            )
            if all_submissions_history_path.exists() and any(
                all_submissions_history_path.iterdir()
            ):
                artifact_all_subs = wandb.Artifact(
                    f"{sanitized_exp_name[:100]}_all_step_submissions", type="all-submissions"
                )
                artifact_all_subs.add_dir(str(all_submissions_history_path))
                self.wandb_run.log_artifact(artifact_all_subs)
                self.app_logger.info("W&B: Logged all_step_submissions artifact.")

            # Main log files for the run
            for log_file_name in [
                "aide.log",
                "aide.verbose.log",
                "final_wandb_summary.csv",
            ]:  # Add summary CSV
            ...
            reports_to_save_directly = [
                "calculated_metrics_results.json",
