...
def trim_long_string(string: str, threshold: int = 3000, k: int = 1200) -> str:  # Reduced defaults
    ...
    if len(string) > threshold:
        ...
        truncated_len = len(string) - (
            len(first_k_chars) + len(last_k_chars)
        )  # More accurate truncated_len
...
def extract_code(text: str, expect_single_script: bool = True) -> str:
    ...
    for pattern in patterns:
        ...
        for match_content in matches:
            ...
            code_candidate = (
                ...
                else "".join(m for m in match_content if m)
            )  # Join if it's a tuple of groups
    ...
    if not extracted_code_blocks and cleaned_text.strip():
        ...
        if lines and (
            lines[0].strip().startswith(("import ", "from ", "def ", "class "))
            or any(line.strip().startswith("# Thought:") for line in lines[:5])
    ...
    for block in extracted_code_blocks:
        if is_valid_python_script(block):
            ...
            print(
                f"Warning: Regex extracted a block, but it's not valid Python:\n---\n{block[:200]}...\n---"
            )
...
def extract_reflection_summary_and_revised_code(text: str) -> tuple[str, str]:
    ...
    if summary_match:
        ...
        summary_match_alt = re.search(
            r"Reflection Summary:(.*?)Revised Code Snippet:", text, re.DOTALL | re.IGNORECASE
        )
        if summary_match_alt:
            summary = summary_match_alt.group(1).strip()
        else:  # Try to grab anything before code as summary
            summary_match_generic = re.search(
                r"(.*?)Revised Code Snippet:", text, re.DOTALL | re.IGNORECASE
            )
