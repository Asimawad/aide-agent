...
from typing import Callable, Optional, Tuple, Type  # Added Type
    ...
    try:
        ...
        if isinstance(e, openai.BadRequestError):
            ...
            is_context_length_error = (
                "maximum context length" in error_message_from_body.lower()
                or "maximum context length" in error_message_attr.lower()
                or "maximum context length" in full_error_str.lower()
                or "context_length_exceeded" in full_error_str.lower()  # Some APIs use this code
            )

            if is_context_length_error:
                logger.error(
                    f"Context length exceeded: {full_error_str}. Not retrying with backoff."
                )
                # Re-raise as a custom error to stop backoff and allow specific handling upstream
                raise ContextLengthExceededError(
                    f"Context length exceeded: {full_error_str}"
                ) from e

        logger.info(
            f"Backoff-eligible exception: {type(e).__name__}: {str(e)}. Full error details: {repr(e)}. Retrying..."
        )
        return False  #
    except Exception as e:
        logger.error(
            f"Non-retryable exception during create_fn: {type(e).__name__}: {str(e)}. Full error: {repr(e)}",
            exc_info=True,
        )
        raise
...
def compile_prompt_to_md(
    prompt: PromptType, _header_depth: int = 1, is_list_item: bool = False
) -> str:
    if isinstance(prompt, str):
        ...
        for item in prompt:
            if isinstance(item, str):
                list_items_compiled.append(
                    compile_prompt_to_md(item, _header_depth, is_list_item=True)
                )
            else:
                compiled_item = compile_prompt_to_md(
                    item, _header_depth
                )  # Keep same header depth or indent?
        ...
        for k, v in prompt.items():
            ...
            indented_compiled_value = "\n".join(
                [f"{indentation}{line}" for line in compiled_value.splitlines()]
            )
            out.append(indented_compiled_value)
        return "\n".join(out).strip() + "\n"  # Ensure a trailing newline for dicts
    else:
        logger.warning(
            f"compile_prompt_to_md received an unexpected type: {type(prompt)}. Value: {prompt}"
        )
        return str(prompt) + "\n"
