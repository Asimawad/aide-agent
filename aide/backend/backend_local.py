...
try:
    ...
    if aide_ds_root.is_dir() and (aide_ds_root / "aide").is_dir():
        ...
        print(
            f"Warning: Could not reliably find aide-ds root from {script_dir_backend}. Imports might fail."
        )
...
class LocalLLMManager:
    ...
    @classmethod
    def get_model(
        cls, model_name: str, load_in_4bit: bool = True
        ...
        if model_name not in cls._cache:
            ...
            try:
                ...
                tokenizer.padding_side = (
                    "left"  # Important for batch generation if ever implemented
                )
                ...
                if load_in_4bit and torch.cuda.is_available():
                    ...
                    logger.info(
                        "Using 4-bit quantization (BitsAndBytesConfig).",
                        extra={"verbose": True},
                    )
    ...
    def generate_response(
        ...
        **gen_kwargs: Any,
        ...
        latency = t1 - t0
        logger.info(
            f"Batch generation of {num_responses} responses and {len(output_ids)} tokens took {t1-t0:.2f} seconds."
        )
...
def query(
    ...
    **model_kwargs: Any,
    ...
    try:
        ...
        logger.debug(
            f"Generating with params:  num_return_sequences = {num_responses}, {model_kwargs}",
            extra={"verbose": True},
        )
...
def process_and_execute_responses(
    ...
    for i, response in enumerate(responses):
        ...
        console.print(
            ...
            + ("..." if response and len(response) > 1000 else "")
        )
        console.print("-" * 20)
        ...
        if response:  # Only process if response is not None
            ...
            if not extracted_code:
                ...
                try:
                    ...
                    formatted_extracted_code = (
                        extracted_code  # Use unformatted for execution attempt
                    )
                    console.print(
                        f"[bold yellow]Warning: Saved unformatted code for response {i+1}.[/bold yellow]"
                    )
        ...
        if formatted_extracted_code:
            ...
            exec_result = ExecutionResult(
                term_out=["Input response was None."],
                exec_time=0,
                exc_type="InputError",
                exc_info={},
                exc_stack=[],
        ...
        if exec_result is None:
            ...
            logger.error(
                f"ExecutionResult was unexpectedly None for response {i+1}. Creating error result."
            )
            exec_result = ExecutionResult(
                term_out=["Unknown processing error."],
                exec_time=0,
                exc_type="ProcessingError",
                exc_info={},
                exc_stack=[],
            )
